...
import { analyzePromptOpenAI } from "@/lib/openai";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await analyzePromptOpenAI(prompt, callerKey, openaiModel, targetModelId);
    return NextResponse.json(result);
  } catch (err: unknown) {
    console.error("[/api/analyze-openai]", err);
